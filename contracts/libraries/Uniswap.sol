...
library Uniswap {
    ...
    /**
     * @notice Compute fees accrued since last poke. 
     * @dev Total earned fees = tokensOwed + @param fee
     */
    ...
    function computeFeesEarned(
        ...
        if (isZero) {
            feeGrowthGlobal = position.pool.feeGrowthGlobal0X128();
            (, , feeGrowthOutsideLower, , , , , ) = position.pool.ticks(position.lower);
            (, , feeGrowthOutsideUpper, , , , , ) = position.pool.ticks(position.upper);
        } else {
            feeGrowthGlobal = position.pool.feeGrowthGlobal1X128();
            (, , , feeGrowthOutsideLower, , , , ) = position.pool.ticks(position.lower);
            (, , , feeGrowthOutsideUpper, , , , ) = position.pool.ticks(position.upper);
        }
